...
export async function generatePayrollPayslips(input: {
  ...
  const payslips: CreatePayslipInput[] = employees.map((e) => {
    ...
    // SKBBK opt-in for this employee-on-this-run, read LIVE from the
    // profile — same as every other statutory setting (EPF rate, SOCSO
    // category, PCB). A recompute is only reachable on a DRAFT run (see
    // the status guard at the top of this function), and nothing on a
    // draft has been remitted, so the current toggle is what should
    // apply. Written back to `Payslip.contributeToSkbbk` as the
    // historical record of what was decided for this payslip; already-
    // submitted payslips keep their frozen value because submitted runs
    // can't be recomputed at all.
    const effectiveContributeToSkbbk = e.profile.contributeToSkbbk
