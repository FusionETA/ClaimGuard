            ...
            <div key={it.id} className="flex items-center justify-between gap-4 px-5 py-3.5">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <p className="truncate font-semibold text-foreground">{meta.title}</p>
              ...
              <Button asChild size="sm" className="shrink-0">
                <Link href={`/employee/appraisals/${it.id}`}>
...
function OverviewTab({
    ...
    return (
      <div className="p-10 text-center">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-surface-low">
          <Icon name="assignment" className="text-3xl text-muted-foreground" />
        </div>
        <h3 className="text-lg font-bold text-foreground">No active appraisal</h3>
  ...
  return (
    <div>
      {/* Status row */}
      <div className="flex flex-col justify-between gap-6 border-b border-border/60 p-6 md:flex-row md:items-center">
      ...
      {complete ? (
        <div className="grid grid-cols-1 gap-4 p-6 md:grid-cols-3">
          ...
          <ScoreCard label="Reviewer 1 Score" icon="visibility" score={scores.reviewer} accent="text-indigo-500 bg-indigo-500/10" bar="bg-indigo-500" loading={loading} />
          <ScoreCard label="Reviewer 2 Score" icon="handshake" score={scores.partner} accent="text-emerald-500 bg-emerald-500/10" bar="bg-emerald-500" loading={loading} />
        </div>
      ) : null}
    </div>
  )
...
function ScoreCard({
  ...
  return (
    <Card className="bg-surface-low p-5 shadow-none">
      ...
      <div className="mb-3 text-3xl font-black text-foreground">
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 bg-surface-low/40 px-6 py-3">
        <div className="relative">
          <Icon
            name="search"
            className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[15px] text-muted-foreground"
          />
          <input
            value={search}
            ...
            className="w-36 rounded-lg border border-border/80 bg-card py-1.5 pl-8 pr-3 text-xs focus:border-primary focus:ring-2 focus:ring-primary"
          />
        </div>
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value as AppraisalStage | "")}
          className="rounded-lg border border-border/80 bg-card px-3 py-1.5 text-xs text-foreground focus:ring-2 focus:ring-primary"
        >
        ...
        >
          ...
          <option value="reviewer">Reviewer 1</option>
          <option value="partner">Reviewer 2</option>
        </select>
      </div>
                ...
                return (
                  <tr key={it.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-4 text-slate-600">
                      {it.viewerPhase === "reviewee"
                        ? "Self"
                        : it.viewerPhase === "reviewer"
                          ? "Reviewer 1"
                          : it.viewerPhase === "partner"
                            ? "Reviewer 2"
                            : "—"}
                    </td>
                    <td className="px-4 py-4">
