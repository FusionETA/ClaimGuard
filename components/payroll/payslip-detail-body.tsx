...
/**
 * The rendered payslip — earnings, deductions, salary basis, plus the
 * print-only header and print stylesheet. Presentational only (no
 * server-only imports), so it renders both from the server detail page
 * and from the client-side reveal gate once the password checks out.
 */
export function PayslipDetailBody({ payslip, run }: PayslipDetailData) {
      ...
      <Card data-payslip-card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          {reimbursements.length > 0 && (
            <LineItemGroup label="Reimbursements" items={reimbursements} />
          )}
